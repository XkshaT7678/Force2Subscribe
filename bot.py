#    This file is part of the ForveSub distribution (https://github.com/Akshat7678/Force2Subscribe).
#    Copyright (c) 2021 Akshat7678
#    
...
# 
#    License can be found in < https://github.com/Akshat7678/Force2Subscribe/blob/main/License> .
...
if (apiid != None and apihash!= None and bottoken != None):
    try:
        Arjvps = TelegramClient('Arjvps', apiid, apihash).start(bot_token=bottoken)
...
async def get_user_join(id):
    ok = True
    try:
        await Arjvps(GetParticipantRequest(channel=channel, participant=id))
        ok = True
...
async def _(event):
    ...
    if event.user_joined or event.user_added:
        ...
        if x is True:
            ...
            await Arjvps.edit_permissions(event.chat.id, user.id, until_date=None, send_messages=False)
        
        await event.reply(msg, buttons=butt)


@Arjvps.on(events.NewMessage(incoming=True))
async def mute_on_msg(event):
    ...
    temp = await Arjvps(GetFullUserRequest(event.sender_id))
    if x is False:
        if temp.user.bot:
            return
        nm = temp.user.first_name
        try:
            await Arjvps.edit_permissions(event.chat.id, event.sender_id, until_date=None, send_messages=False)
        except Exception as e:
            print(str(e))
            return
...
@Arjvps.on(events.callbackquery.CallbackQuery(data=re.compile(b"unmute_(.*)")))
async def _(event):
    uid = int(event.data_match.group(1).decode("UTF-8"))
    if uid == event.sender_id:
        x = await get_user_join(uid)
        nm = (await Arjvps(GetFullUserRequest(uid))).user.first_name
        if x is False:
            await event.answer(f"You haven't joined @{channel} yet!", cache_time=0, alert=True)
        elif x is True:
            try:
                await Arjvps.edit_permissions(event.chat.id, uid, until_date=None, send_messages=True)
            except Exception as e:
                print(str(e))
                return
...
@Arjvps.on(events.NewMessage(pattern="/start"))
async def strt(event):
    await event.reply(f"Hi. I'm a force subscribe bot made specially for @{channel}!\n\nCheckout @TheArjvps for Updates..", buttons=[Button.url("Channel", url=f"https://t.me/{channel}"), Button.url("Repository", url="https://github.com/Akshat7678/Force2Subscribe")])

    
print("ForceSub Bot has started.\nDo visit @TheArjvps!")
Arjvps.run_until_disconnected()
